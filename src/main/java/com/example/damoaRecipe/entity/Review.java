...
public class Review{
    ...
    public static Review createReview(ReviewForm form, Recipe recipe) {
        if(form.getReviewId()!=null) throw new IllegalArgumentException("댓글 생성 실패! 댓글의 id가 없어야합니다.");
        if(form.getRecipeId()!=recipe.getRecipeId()) throw new IllegalArgumentException("댓글 생성 실패! 게시글의 id가 잘못됐습니다.");
